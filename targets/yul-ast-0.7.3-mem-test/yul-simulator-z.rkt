...
(define yul-simulator%
	...
	(class object%
		...
		(define/public (set-calldatasize t)
			(set! yul-calldatasize t)
		)
		(define/public (set-returndatasize t)
			(set! yul-returndatasize t)
		)
		(define/public (set-gas t)
			(set! yul-gas t)
		)
		...
		(define (interpret-node args-register arg-node #:block-create-scope [block-create-scope #t])
			...
			(match arg-node
				...
				[(list "yul_function_definition" m-identifier ms-function-xxx-list ... m-block)
					...
					; then interpret
					(define jr (interpret-node args-register m-block))
					(if (null? p-ret-list)
						; (FIXME) no ret list is found, return jr
						jr
				...
				[(list-rest "yul_block" ms-statement)
					...
					(define r-stay #t)
					(for/last ([m-statement ms-statement])
		...
		(define (initialize-builtin-functions)
			...
			; 	; (fixme) not sure whether 32 interval is right
			; 	(define res (for/list ([p0 (range (bitvector->natural p) (+ (bitvector->natural p) (bitvector->natural s) 32) 32)])
			; 		(read-var null (bv p0 yul-default-bitvector) #:where "m")
			; 	))
			; 	res
			...
			; fixme
			(hash-set! yul-functions "delegatecall" (lambda (p0 p1 p2 p3 p4 p5)
				(assert #t)
			))
			; fixme
			(hash-set! yul-functions "gas" (lambda ()
			...
			; fixme
			(hash-set! yul-functions "returndatasize" (lambda ()
				; (bv 32 yul-default-bitvector)
				yul-returndatasize
			))
			; fixme
			(hash-set! yul-functions "returndatacopy" (lambda (t f s)
				(assert #t)
			))
			; fixme
