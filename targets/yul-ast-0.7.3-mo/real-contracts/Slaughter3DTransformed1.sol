...
contract Slaughter3DTransformed1 {
struct Stage {
	uint8 numberOfPlayers;
	uint256 blocknumber;
	bool finalized;
	mapping(uint8 => A) slotXplayer;
	mapping(address => bool) players;
}

struct A {
	/* t1 ->*/ address t1address_0_WRAP;
	/* t2 ->*/ address t2address_1_WRAP;
}
...
player = msg.sender;
if (false) {
stages[stageIndex].slotXplayer[stages[stageIndex].numberOfPlayers].t1address_0_WRAP = player;
}
stages[stageIndex].numberOfPlayers++;
stages[stageIndex].players[player] = true;
if (false) {
stages[stageIndex].slotXplayer[stages[stageIndex].numberOfPlayers].t2address_1_WRAP = MN;
}
...
address survivor;
survivor = stages[numberOfFinalizedStages].slotXplayer[i++].t1address_0_WRAP;
...
while ((i < MAX_PLAYERS_PER_STAGE)) {
address player;
player = stages[stageIndex].slotXplayer[i++].t1address_0_WRAP;
...
return (stages[numberOfFinalizedStages].slotXplayer[0].t1address_0_WRAP);
}
function observe__2() public view returns (address) {
return (stages[numberOfFinalizedStages].slotXplayer[2].t1address_0_WRAP);
}
function observe__3() public view returns (address) {
return (stages[numberOfStages].slotXplayer[0].t1address_0_WRAP);
}
function observe__4() public view returns (address) {
return (stages[numberOfStages].slotXplayer[1].t1address_0_WRAP);
}
...
return stages[numberOfStages].numberOfPlayers;
}
