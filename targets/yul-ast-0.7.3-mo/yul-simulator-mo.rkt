	...
	(class object%
		...
		(define (initialize-builtin-functions)
			...
			(hash-set! yul-functions "keccak256" (lambda (p n)
				(println-and-exit (format "# [exit] keccak256 is blocked in the memory-optimized version.\n"))
			))
			; fixme
			...
			; memory-optimized version special function
			(hash-set! yul-functions "mo-mia" (lambda (p n)
				(mo-mia p n)
			))
